...
/**
 * Admin-portal fetch: always uses the staff token (bts_staff_access_token) — this app
 * has no customer surface, so there is no dual-token branching here. A 401 (expired
...
async function handleApiResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    // Every admin endpoint demands an admin session: 401 and 403 both mean the current
    // credential can't drive this portal — drop it and redirect to the admin login.
