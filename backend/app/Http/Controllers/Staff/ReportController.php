...
class ReportController extends Controller
{
    ...
    public function scheduleAppointment(Request $request, CreditApplication $application): JsonResponse
    {
        ...
        $branchId = $validated['branch_id'] ?? $application->branch_id;
        $branch = $branchId ? Branch::find($branchId) : ($application->branch ?? Branch::first());
        ...
        if ($this->stateMachine->canTransition($application, $targetStatus, $request->user())) {
        ...
        $attemptNumber = ($application->appointments()->max('attempt_number') ?? 0) + 1;
        $appointment = $application->appointments()->create([
            'branch_id' => $branch->id,
            'scheduled_date' => $validated['scheduled_date'],
            'scheduled_time' => $validated['scheduled_time'],
            'status' => $directConfirm ? Appointment::STATUS_ACCEPTED : Appointment::STATUS_PROPOSED,
