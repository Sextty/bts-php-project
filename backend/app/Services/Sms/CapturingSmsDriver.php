 ...
 * pull the plaintext OTP straight from the captured code — otp_codes only ever stores the hash,
...
{
    /** @var array<int, array{to: string, code: string}> */
    public array $sent = [];
    ...
    public function send(User $user, OtpMessage $message): SmsDeliveryResult
    {
        ...
        $this->sent[] = ['to' => (string) $user->phone, 'code' => $message->code];
