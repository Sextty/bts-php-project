...
class AppointmentConcurrencyTest extends TestCase
{
    ...
    public function test_mariadb_serializes_slot_capacity_application_and_attempt_races(): void
    {
        ...
        $sameSlot = $this->runConcurrent([
            [$appA->id, $staffA->id, $branch->id, '2026-08-27', '10:00'],
            [$appB->id, $staffB->id, $branch->id, '2026-08-27', '10:00'],
        ]);
        $this->assertSame(1, collect($sameSlot)->where('ok', true)->count());
        $this->assertSame(1, Appointment::whereDate('scheduled_date', '2026-08-27')->count());
        ...
        $duplicate = $this->runConcurrent([
            [$sameApplication->id, $staffA->id, $branch->id, '2026-08-28', '11:00'],
            [$sameApplication->id, $staffB->id, $branch->id, '2026-08-28', '11:00'],
        ]);
        ...
        $attempts = $this->runConcurrent([
            [$attemptApplication->id, $staffA->id, $branch->id, '2026-08-31', '09:00'],
            [$attemptApplication->id, $staffB->id, $branch->id, '2026-08-31', '14:00'],
        ]);
        ...
        $capacity = $this->runConcurrent([
            [$this->application($smallBranch)->id, $smallStaffA->id, $smallBranch->id, '2026-09-01', '09:00'],
            [$this->application($smallBranch)->id, $smallStaffB->id, $smallBranch->id, '2026-09-01', '11:00'],
        ]);
        $this->assertSame(1, collect($capacity)->where('ok', true)->count());
        $this->assertSame(1, Appointment::where('branch_id', $smallBranch->id)->whereDate('scheduled_date', '2026-09-01')->count());
    }
