...
export function cancelStaffApplication(id: number) {
  return apiFetch<{ application: StaffApplicationDto }>(`/staff/applications/${id}/cancel`, {
    method: 'POST',
    auth: 'staff',
  });
}
...
export async function downloadStaffDocumentBlob(
  applicationId: number,
  documentId: number
): Promise<{ blob: Blob; mimeType: string }> {
  const token = getStaffToken();
  const response = await fetch(`${API_URL}/api/staff/applications/${applicationId}/documents/${documentId}`, {
    headers: {
      Accept: '*/*',
  ...
  if (!response.ok) {
    throw new Error(`Impossible de charger le document (Erreur HTTP ${response.status})`);
  }
